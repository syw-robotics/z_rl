...
This guide provides an overview of the features and structure of Z-RL. After introducing the currently available
...
:ref:`minimal example <example-integration>` of how to integrate Z-RL into a project.
...
Z-RL is intentionally kept minimal and focuses on a small set of components that cover common robotics workflows while
remaining easy to adapt. The following sections summarize the main features currently available.

Algorithms
^^^^^^^^^^

:class:`~z_rl.algorithms.ppo.PPO`
...
:class:`~z_rl.algorithms.distillation.Distillation`
...
:class:`~z_rl.models.mlp_model.MLPModel`
...
:class:`~z_rl.models.rnn_model.RNNModel`
   A recurrent extension of the :class:`~z_rl.models.mlp_model.MLPModel` for partially observable settings. The 
...
:class:`~z_rl.models.cnn_model.CNNModel`
   ...
   independently. When used in conjunction with :class:`~z_rl.algorithms.ppo.PPO`, the encoders may be shared between
   actor and critic to save memory.

Distributions
^^^^^^^^^^^^^

:class:`~z_rl.modules.distribution.GaussianDistribution`
...
:class:`~z_rl.modules.distribution.HeteroscedasticGaussianDistribution`
...
Symmetry
   ...
   :class:`~z_rl.algorithms.ppo.PPO` algorithm. For more details, please check
...
Z-RL consists of four core components: :doc:`Runners <../api/runners>`, :doc:`Algorithms <../api/algorithms>`,
...
.. figure:: ../_static/z_rl_structure_light.svg
   :width: 100%
   :align: center
   :class: light-only


.. figure:: ../_static/z_rl_structure_dark.svg
   :width: 100%
   :align: center
   :class: dark-only

.. note::
   In the following, functions are linked to implementations for a standard :class:`~z_rl.algorithms.ppo.PPO` training
   pipeline. Other learning pipelines may have different implementations of these functions.

Runners
^^^^^^^
The **Runner** implements the main learning loop in :func:`~z_rl.runners.on_policy_runner.OnPolicyRunner.learn` and
coordinates the interaction with the **Environment**. It is the main API of the library, providing functionality for
saving, loading, and exporting checkpoints and models via :func:`~z_rl.runners.on_policy_runner.OnPolicyRunner.save`,
:func:`~z_rl.runners.on_policy_runner.OnPolicyRunner.load`,
:func:`~z_rl.runners.on_policy_runner.OnPolicyRunner.export_policy_to_jit`,
and :func:`~z_rl.runners.on_policy_runner.OnPolicyRunner.export_policy_to_onnx`. Additionally, it coordinates the
initialization of the multi-GPU pipeline, the **Algorithm**, and the :class:`~z_rl.utils.logger.Logger`.

Algorithms
^^^^^^^^^^
The **Algorithm** implements the logic of the core learning process. It is responsible for acting on the observations
provided by the **Environment** in :func:`~z_rl.algorithms.ppo.PPO.act`, processing data collected from the
environment interaction in :func:`~z_rl.algorithms.ppo.PPO.process_env_step`, and updating the parameters of the
learnable **Model** instances in :func:`~z_rl.algorithms.ppo.PPO.update`. The **Algorithm** can make use of a
...
:func:`~z_rl.algorithms.ppo.PPO.construct_algorithm`.

Models
^^^^^^
The **Model** implements a neural network that is used by the **Algorithm** for different purposes. For example,
:class:`~z_rl.algorithms.ppo.PPO` uses **Model** instances for the actor and critic networks, while
:class:`~z_rl.algorithms.distillation.Distillation` uses **Model** instances for the student and teacher networks. The
:func:`~z_rl.models.mlp_model.MLPModel.forward` pass of a **Model** instance consists of three steps:

1. A latent is computed from raw observations in :func:`~z_rl.models.mlp_model.MLPModel.get_latent`. This may involve
   normalization, recurrent processing for :class:`~z_rl.models.rnn_model.RNNModel` instances, or convolutional
   encoding for :class:`~z_rl.models.cnn_model.CNNModel` instances.
...
3. The output of the MLP is either returned as is or used to sample from a
   :class:`~z_rl.modules.distribution.Distribution`, in case stochastic outputs are requested.
...
:class:`~z_rl.modules.mlp.MLP`, a normalizer, such as
:class:`~z_rl.modules.normalization.EmpiricalNormalization`, or a distribution, such as the
:class:`~z_rl.modules.distribution.GaussianDistribution`. Modules are initialized in the **Model** constructor and
...
Next to some required attributes, the **Environment** must implement the :func:`~z_rl.env.vec_env.VecEnv.step`
and :func:`~z_rl.env.vec_env.VecEnv.get_observations` methods.

Storage
^^^^^^^
The **Storage** implements a storage buffer that is used by the **Algorithm** to store collected environment interaction
data. It returns the data to the **Algorithm** for its :func:`~z_rl.algorithms.ppo.PPO.update` in a suitable format,
for example in mini-batches.

Extensions
^^^^^^^^^^
An **Extension** implements an augmentation to a specific **Algorithm** to modify its behavior. Currently, Z-RL does 
not constrain the way an **Extension** may be implemented, allowing for arbitrary modifications to the learning process.

Utils
^^^^^
**Utils** include various helpers for the library, such as a :class:`~z_rl.utils.logger.Logger` to record the learning
...
initializes an :class:`~z_rl.runners.on_policy_runner.OnPolicyRunner`, 4) runs the training, and 5) exports the
trained policy for deployment. The environment is defined by the user and must implement the
:class:`~z_rl.env.vec_env.VecEnv` interface. The configuration setup is described in the
...
.. code-block:: python
   ...
   # 2) Load a YAML configuration and extract the configuration dictionary expected by Z-RL
