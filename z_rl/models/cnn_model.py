...
from z_rl.modules import CNN, HiddenState


class CNNModel(MLPModel):
    """CNN-based neural model.

    Data flow: ``obs groups -> (split 1D/2D) -> (concat 1D -> normalization) + (CNN encodes for 2D) -> concat latent -> head -> (distribution) -> output``.
